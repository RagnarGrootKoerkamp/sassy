use crate::profiles::{u8x32_gt, Profile};
...
pub fn ascii_u64_search(seq: &[u8; 64], bases: &[u8], out: &mut [u64]) {
    unsafe {
        ...
        for (i, &base) in bases.iter().enumerate() {
            ...
            let low = eq0.move_mask() as u32 as u64;
            let high = eq1.move_mask() as u32 as u64;
...
fn ascii_u64_search_case_insensitive(seq: &[u8; 64], bases: &[u8], out: &mut [u64]) {
    unsafe {
        ...
        let is_char0 = u8x32_gt(chunk0, u8x32::splat(A-1))  & u8x32_gt(u8x32::splat(Z+1), chunk0);
        let is_char1 = u8x32_gt(chunk1, u8x32::splat(A-1))  & u8x32_gt(u8x32::splat(Z+1), chunk1);
        // Transmute from i8x32 to u8x32
        let lower0 =
            chunk0 | (u8x32::splat(to_lowercase) & is_char0);
        let lower1 =
            chunk1 | (u8x32::splat(to_lowercase) & is_char1);

        for (i, &base) in bases.iter().enumerate() {
            let m = u8x32::splat(base | 0x20);
            let eq0 = lower0.cmp_eq(m);
            let eq1 = lower1.cmp_eq(m);
            let low = eq0.move_mask() as u32 as u64;
            let high = eq1.move_mask() as u32 as u64;
            *out.get_unchecked_mut(i) = (high << 32) | low;
        }
