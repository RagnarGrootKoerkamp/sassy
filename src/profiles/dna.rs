...
impl Profile for Dna {
    ...
    fn encode_ref(&self, b: &[u8; 64], out: &mut Self::B) {
        unsafe {
            ...
            for (i, code) in CODES.iter().enumerate() {
                let eq0 = bases0.cmp_eq(*code);
                let eq1 = bases1.cmp_eq(*code);
    ...
    fn valid_seq(seq: &[u8]) -> bool {
        ...
        while i + LANES <= len {
            ...
            let is_a = lowered.cmp_eq(u8x32::splat(b'a'));
            let is_c = lowered.cmp_eq(u8x32::splat(b'c'));
            let is_g = lowered.cmp_eq(u8x32::splat(b'g'));
            let is_t = lowered.cmp_eq(u8x32::splat(b't'));
