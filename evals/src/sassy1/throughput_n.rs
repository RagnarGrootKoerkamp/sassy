...
struct Config {
    ...
    ks: Vec<f64>,
    /// Number of query sequences per (target_len, k). More = more stable throughput.
    #[serde(default)]
    n_queries: Option<usize>,
    /// Number of reference/text sequences per (target_len, k). More = more stable throughput.
    #[serde(default)]
    n_refs: Option<usize>,
    min_benchtime: f64,
...
pub fn run(config_path: &str) {
    ...
    println!("Query length: {}", config.query_len);
    ...
    let query_len = config.query_len;
