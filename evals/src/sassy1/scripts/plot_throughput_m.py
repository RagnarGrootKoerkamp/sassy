#!/usr/bin/env python3
"""Plot throughput vs pattern length from throughput_m benchmark CSV (search_throughput_pat_len format)."""
import argparse
import math
import os
...
DEFAULT_CSV = os.path.join(SCRIPT_DIR, "..", "output", "search_throughput_pat_len.csv")
...
def _draw_plot(ax, df, line_style_map, include_parasail):
    """Draw throughput vs query_len on ax. include_parasail: add Parasail series if column exists."""
    sassy_color = "#fcc007"
    edlib_color = "black"
    ...
    unique_query_lens = sorted(df["query_len"].unique())
    ...
    legend_labels = (
        ["Sassy", "Edlib"] + (["Parasail"] if (include_parasail and "parasail_throughput_gbps" in df.columns) else [])
...
def main():
    parser = argparse.ArgumentParser(description="Plot throughput vs pattern length")
    parser.add_argument(
        "csv",
        nargs="?",
        default=DEFAULT_CSV,
    ...
    df = (
        df.groupby(["query_len", "k"], as_index=False)
        .agg(agg_dict)
        .sort_values(["k", "query_len"])
    )
    ...
    plt.sca(ax1)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "throughput_m.svg"), bbox_inches="tight")
    plt.savefig(os.path.join(out_dir, "throughput_m.pdf"), bbox_inches="tight")
    plt.close(fig1)

    # Plot 2: Sassy + Edlib + Parasail
    fig2, ax2 = plt.subplots(figsize=(8, 6))
    _draw_plot(ax2, df, line_style_map, include_parasail=True)
    plt.sca(ax2)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "throughput_m_parasail.svg"), bbox_inches="tight")
    plt.savefig(os.path.join(out_dir, "throughput_m_parasail.pdf"), bbox_inches="tight")
    plt.close(fig2)

    print(f"Plots saved to {out_dir}/ (throughput_m.* and throughput_m_parasail.*)")
    return 0
