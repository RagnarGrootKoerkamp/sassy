...
if os.path.exists(scaling_csv):
    ...
    df_scaling = (
        df_scaling
        .groupby(["k", "query_len", "algorithm", "target_len"], as_index=False)
    ...
    for k in k_values:
        for query_len in query_lengths:
            ...
            for algorithm in ["sassy_search", "sassy_tiling", "edlib"]:
                ...
                color = {"sassy_search": sassy_search_color,
                         "sassy_tiling": sassy_tiling_color,
                         "edlib": edlib_color}[algorithm]
    ...
    ax1.legend(handles=legend_handles_a, loc='lower right',
               frameon=True, fancybox=False, framealpha=0.9,
               edgecolor='#CCCCCC', handlelength=2.5, handletextpad=0.5,
               fontsize=6)
...
if os.path.exists(throughput_csv):
    ...
    # median aggregation
    df_throughput = df_throughput.groupby("num_queries", as_index=False).median(numeric_only=True)

    tools = ['search', 'tiling', 'edlib']
    tool_colors = {'search': sassy_search_color, 'tiling': sassy_tiling_color, 'edlib': edlib_color}
    ...
    legend_handles_b = [
        Line2D([0], [0], color=tool_colors[tool], linestyle='--',
               marker=tool_markers[tool], markersize=3,
               markeredgewidth=0.5, lw=1.2,
               label=f"{tool_labels[tool]} (k=3)")
        for tool in tools
    ]
    ...
    ax2.legend(handles=legend_handles_b, loc='upper left',
               frameon=True, fancybox=False, framealpha=0.9,
               edgecolor='#CCCCCC', handlelength=1.5, handletextpad=0.3,
               fontsize=6)
